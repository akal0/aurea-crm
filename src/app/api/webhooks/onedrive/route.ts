...
import { enqueueOneDriveNotification } from "@/features/onedrive/server/subscriptions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const notification of notifications) {
      if (notification.clientState !== "aurea-crm-onedrive-webhook") {
        continue;
      }

      await enqueueOneDriveNotification({
