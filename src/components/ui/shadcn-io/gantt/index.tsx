...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({
  ...
  return (
    <div
      className="sticky top-0 z-20 grid w-full shrink-0 bg-[#202e32] border-l border-white/5 backdrop-blur-sm"
      style={{ height: "var(--gantt-header-height)" }}
    >
      <div>
        <div
          className="sticky inline-flex whitespace-nowrap px-3 py-2 text-white/50 text-[11px]"
          style={{
      ...
      >
        {Array.from({ length: columns }).map((_, index) => (
          <div
            className="shrink-0 border-white/5 border-b py-1 text-center text-xs"
            key={`${id}-${index}`}
          >
          ...
          <GanttContentHeader
            ...
            renderHeaderItem={(item: number) => (
              <div className="flex items-center justify-center gap-4 flex-col">
                <p className="absolute top-6 text-[10px]">
                  {format(addDays(new Date(year.year, index, 1), item), "d")}
                </p>

                <p className="text-white/50 text-[10px]">
                  {format(addDays(new Date(year.year, index, 1), item), "EEE")}
                </p>
              </div>
            )}
            title={`${format(new Date(year.year, index, 1), "MMMM yy")}'`}
          />
...
export const GanttHeader: FC<GanttHeaderProps> = ({ className }) => {
  ...
  return (
    <div
      className={cn(
        "-space-x-px flex h-full w-max divide-x divide-white/5",
        className,
      )}
    >
      <Header />
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex items-center gap-2.5 p-2.5 pl-6 text-xs hover:bg-[#202e32] hover:brightness-110 transition duration-150",
        className,
      )}
      key={feature.id}
      onClick={handleClick}
    ...
    >
      ...
      />
      <p className="pointer-events-none flex-1 truncate text-left font-medium text-[11px]">
        {feature.name}
      </p>
      <p className="pointer-events-none text-white/50 text-[10px] pr-6">
        {duration}
      </p>
    </div>
  );
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-b border-white/5 bg-[#202e32] p-6 pb-4  font-medium text-white text-[10px] backdrop-blur-sm"
    style={{ height: "var(--gantt-header-height)" }}
  >
    {/* <Checkbox className="shrink-0" /> */}
    <p className="flex-1 truncate text-left">Deals</p>
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <p
      className="w-full truncate p-6 text-left font-medium text-white/50 text-[11px]"
...
export const GanttSidebar: FC<GanttSidebarProps> = ({
  ...
  <div
    className={cn(
      "sticky left-0 z-30 h-max min-h-full overflow-clip border-white/5 border-r bg-[#202e32]/97 backdrop-blur-md text-xs text-white",
      className,
    )}
...
export const GanttColumn: FC<GanttColumnProps> = ({
  ...
  return (
    ...
    <div
      className={cn("group relative h-full overflow-hidden")}
...
export const GanttColumns: FC<GanttColumnsProps> = ({
  ...
  return (
    <div
      className="divide grid h-full w-full divide-x divide-white/5"
      style={{
        gridTemplateColumns: `repeat(${columns}, var(--gantt-column-width))`,
      }}
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="whitespace-nowrap rounded-full border border-white/5 bg-background/90 px-2 py-1 text-black text-xs backdrop-blur-lg">
  ...
  return (
    ...
    >
      ...
      {date && (
        <div
          className={cn(
            "-translate-x-1/2 absolute top-10 hidden whitespace-nowrap rounded-lg border border-white/5 bg-background/90 px-2 py-1 text-black text-xs backdrop-blur-lg group-hover:block",
            isPressed && "block",
          )}
        >
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card className="h-full w-full rounded-sm text-white bg-[#202e32] brightness-120 p-2 text-xs shadow-sm border-white/5">
      <div
        className={cn(
    ...
    >
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <div
            className={cn(
              "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md bg-card px-2 py-1 text-black text-xs",
              className,
            )}
          >
            {label}
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "gantt relative grid h-full w-full flex-none select-none overflow-hidden rounded-xs bg-[#202e32] border-t border-white/5",
          range,
          className,
        )}
        ref={scrollRef}
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-sm bg-[#202e32] border border-white/5 brightness-140 text-white px-2 py-1 text-[10px]",
          className,
        )}
      >
        {label}
        <span className="max-h-[0] overflow-hidden opacity-50 transition-all group-hover:max-h-[2rem]">
          {formatDate(date, "MMM dd, yyyy")}
        </span>
      </div>
      <div className={cn("h-full w-px bg-white/5", className)} />
    </div>
  );
};
