...
type RangeSliderProps = {
  ...
  prefix?: string; // Deprecated: use minPrefix and maxPrefix instead
  minPrefix?: string;
  maxPrefix?: string;
  suffix?: string;
...
export default function RangeSlider({
  ...
  // Support backward compatibility: if prefix is provided but not minPrefix/maxPrefix, use prefix for both
  const effectiveMinPrefix = minPrefix ?? prefix;
  const effectiveMaxPrefix = maxPrefix ?? prefix;
  const id = useId();
  ...
  const parseDuration = (text: string): number | null => {
    ...
    if (/^\d{1,2}(:\d{1,2}){1,2}$/.test(raw)) {
      const parts = raw.split(":").map((p) => Number.parseInt(p || "0", 10));
      if (parts.length === 3) {
        const [h, m, s] = parts;
  ...
  const formatNumber = (n: number, customPrefix?: string): string => {
    const num = Number(n || 0);
    const negative = num < 0;
    const absStr = Math.abs(num).toLocaleString();
    const pre = customPrefix ?? "";
    const suf = suffix || "";
  ...
  const [formattedMin, setFormattedMin] = useState<string>(
    mode === "duration"
      ...
      : formatNumber(Number(initial[0] || 0), effectiveMinPrefix)
  );
  const [formattedMax, setFormattedMax] = useState<string>(
    mode === "duration"
      ? formatSeconds(Number(initial[1] || 0))
      : formatNumber(Number(initial[1] || 0), effectiveMaxPrefix)
  );
  ...
  const handleSliderValueChange = (values: number[]) => {
    ...
    setFormattedMin(
      mode === "duration"
        ...
        : formatNumber(Number(values[0] || 0), effectiveMinPrefix)
    );
    setFormattedMax(
      mode === "duration"
        ? formatSeconds(Number(values[1] || 0))
        : formatNumber(Number(values[1] || 0), effectiveMaxPrefix)
    );
  };
    ...
    if (Array.isArray(sliderValue) && sliderValue.length >= 2) {
      setFormattedMin(
        mode === "duration"
          ...
          : formatNumber(Number(sliderValue[0] || 0), effectiveMinPrefix)
      );
      setFormattedMax(
        mode === "duration"
          ? formatSeconds(Number(sliderValue[1] || 0))
          : formatNumber(Number(sliderValue[1] || 0), effectiveMaxPrefix)
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
  ...
  return (
    <div className="*:not-first:mt-4 flex flex-col gap-1">
              ...
              return (
                ...
                >
                  <span
                    ...
                    className="block w-full h-full bg-primary/50 brightness-150"
                  />
                </div>
              );
            })}
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="*:not-first:mt-1 flex flex-col gap-1">
          <Label
            htmlFor={`${id}-min`}
            className="text-xs text-center font-normal text-primary/80 dark:text-white/60"
          >
            {minInputLabel ||
          ...
          <Input
              ...
              if (e.key === "Enter") {
                ...
                setFormattedMin(
                  mode === "duration"
                    ...
                    : formatNumber(clamped, effectiveMinPrefix)
                );
                commitCurrent();
                setWaveActive(false);
              } else if (
            ...
            placeholder={
              mode === "duration"
                ...
                : `e.g. ${effectiveMinPrefix || ""}100${suffix || ""}`
            }
            disabled={disabled}
          />
        </div>
        ...
        <div className="*:not-first:mt-1 flex flex-col gap-1">
          <Label
            htmlFor={`${id}-max`}
            className="text-xs text-primary/80 dark:text-white/60 font-normal"
          >
            {maxInputLabel ||
          ...
          <Input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Enter") {
                ...
                setFormattedMax(
                  mode === "duration"
                    ? formatSeconds(clamped)
                    : formatNumber(clamped, effectiveMaxPrefix)
                );
                commitCurrent();
                setWaveActive(false);
              } else if (
            ...
            placeholder={
              mode === "duration"
                ...
                : `e.g. ${effectiveMaxPrefix || ""}1,000${suffix || ""}`
            }
            disabled={disabled}
          />
        </div>
        ...
        {showCountButton ? (
          <Button
            className="w-full border-white/5! rounded-sm bg-transparent! hover:bg-[#202e32]! hover:brightness-120! py-3 text-xs"
            variant="outline"
