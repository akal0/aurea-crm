...
interface ClientMindbodySectionProps {
  client: {
    id: string;
    source?: string | null;
    metadata?: unknown;
...
function readMindbodyMetadata(metadata: unknown): MindbodyMetadata | null {
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) return null;
  const mindbody = (metadata as Record<string, unknown>).mindbody;
  if (!mindbody || typeof mindbody !== "object" || Array.isArray(mindbody)) return null;
  const record = mindbody as Record<string, unknown>;
  return {
    id: typeof record.id === "string" ? record.id : undefined,
    status: typeof record.status === "string" ? record.status : undefined,
    creationDate: typeof record.creationDate === "string" ? record.creationDate : undefined,
    lastModified: typeof record.lastModified === "string" ? record.lastModified : undefined,
  };
}

export function ClientMindbodySection({
  client,
}: ClientMindbodySectionProps) {
  const [isSyncing, setIsSyncing] = useState(false);

  const mindbodyData = readMindbodyMetadata(client.metadata);
  ...
  const handleSync = async () => {
    try {
      setIsSyncing(true);
      // This would call a sync endpoint for this specific client
      // await trpc.mindbody.syncClient.mutate({
      //   clientId: client.id,
      //   mindbodyClientId: mindbodyId
      // });
      toast.success("Sync started", {
        description: "Syncing Mindbody data for this client...",
      });
    } catch (error) {
