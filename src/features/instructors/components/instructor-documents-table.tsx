...
export interface InstructorDocumentsTableProps {
  instructorId: string;
}

export function InstructorDocumentsTable({ instructorId }: InstructorDocumentsTableProps) {
  const trpc = useTRPC();
  ...
  const { data: documents, refetch } = useSuspenseQuery(
    trpc.instructors.getDocuments.queryOptions({ instructorId })
  );

  const deleteDocumentMutation = useMutation(
    trpc.instructors.deleteDocument.mutationOptions({
      onSuccess: () => {
  ...
  const approveDocumentMutation = useMutation(
    trpc.instructors.approveDocument.mutationOptions({
      onSuccess: () => {
  ...
  const rejectDocumentMutation = useMutation(
    trpc.instructors.rejectDocument.mutationOptions({
      onSuccess: () => {
  ...
  const handleDelete = (documentId: string) => {
    if (confirm("Are you sure you want to delete this document?")) {
      deleteDocumentMutation.mutate({ documentId, instructorId });
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Manage instructor compliance documents</CardDescription>
            </div>
          ...
          {filteredDocuments?.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-primary/60 mt-1">
                {filterStatus === "all"
                  ? "This instructor hasn't uploaded any documents yet"
