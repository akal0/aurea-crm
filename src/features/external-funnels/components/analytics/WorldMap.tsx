...
export function WorldMap({ geoPoints }: WorldMapProps) {
      ...
      .then((svgContent) => {
        ...
        if (svgElement) {
          ...
          paths.forEach((path) => {
            ...
            const countryData = geoPoints.find(
              (p) => p.countryCode === countryId,
            );
          ...
          setTimeout(() => {
            ...
            const pathsWithLabels =
              contentRef.current.querySelectorAll("path[id]");
            ...
            geoPoints.forEach((point) => {
              ...
              const countryPath = contentRef.current?.querySelector(
                `path[id="${point.countryCode}"]`,
              ) as SVGPathElement;
              if (countryPath) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-full bg-white rounded-lg overflow-hidden min-h-[800px]"
    >
      {isLoading && (
      ...
      {hoveredCountry && (
        <div className="absolute top-4 left-4 bg-white border border-gray-200 rounded-lg px-4 py-2 pointer-events-none z-10 shadow-lg">
          ...
          <div className="text-xs text-gray-500">
            {geoPoints.find((p) => p.country === hoveredCountry)?.count || 0}{" "}
            sessions
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1">
            <div
              ...
              style={{ backgroundColor: getHeatColor(0.2) }}
            />
          ...
          <div className="flex items-center gap-1">
            <div
              className="w-4 h-4 rounded-sm border border-gray-300"
              style={{ backgroundColor: getHeatColor(1.0) }}
            />
