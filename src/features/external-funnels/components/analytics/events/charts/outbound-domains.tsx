...
type OutboundDomainsChartProps = {
  funnelId: string;
  timeRange: "24h" | "7d" | "30d" | "90d";
};

export function OutboundDomainsChart({
  funnelId,
  timeRange,
}: OutboundDomainsChartProps) {
  const trpc = useTRPC();
  ...
  if (data.totalEvents === 0) {
    return (
      <Card className="rounded-none shadow-none border-x-0 border-t-0">
        <CardHeader>
          <CardTitle className="text-sm">Outbound domains</CardTitle>
  ...
  const destinationProperty = data.properties.find(
    (prop) => prop.propertyKey === "destination"
  );
  ...
  return (
    <Card
      className="rounded-none shadow-none border-x-0 border-y-0 md:col-span-1 pb-0"
      ref={ref}
    >
      <CardHeader>
        <div>
      ...
      <CardContent className="px-0">
            ...
            return (
              ...
              >
                <motion.div
                  ...
                  style={{
                    backgroundColor: `rgba(16, 185, 129, ${bgOpacity * 0.12})`,
                  }}
                />
        ...
        {breakdown.length === 0 && (
          <div className="flex items-center justify-center py-12 text-sm text-muted-foreground">
            No outbound domain data available
          </div>
        )}
      </CardContent>
    </Card>
