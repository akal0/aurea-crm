...
export interface SessionsToolbarProps {
  ...
  onConvertedFilterChange?: (value: string) => void;
  // Date filters
  startedAtStart?: Date;
  startedAtEnd?: Date;
  onStartedAtChange?: (start?: Date, end?: Date) => void;
  // Sort
  sortValue?: string;
...
const sortOptions = [
  { value: "startedAt.desc", label: "Most recent" },
  { value: "startedAt.asc", label: "Oldest first" },
  { value: "pageViews.desc", label: "Most page views" },
  { value: "pageViews.asc", label: "Fewest page views" },
  { value: "eventsCount.desc", label: "Most events" },
  { value: "eventsCount.asc", label: "Fewest events" },
...
export function SessionsToolbar({
  ...
  const [stagedDeviceTypes, setStagedDeviceTypes] = React.useState<string[]>(
    selectedDeviceTypes
  );
  const [stagedConvertedFilter, setStagedConvertedFilter] =
    React.useState<string>(convertedFilter);
  ...
  React.useEffect(() => {
    setStagedConvertedFilter(convertedFilter);
  }, [convertedFilter]);
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                      <Checkbox
                        checked={stagedConvertedFilter === ""}
                        onCheckedChange={() => setStagedConvertedFilter("")}
                        className="rounded-lg border-black/5 dark:border-white/5"
                      />
                      <span className="text-xs text-primary">All sessions</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Checkbox
                        checked={stagedConvertedFilter === "true"}
                        onCheckedChange={() =>
                          setStagedConvertedFilter("true")
                        }
                        className="rounded-lg border-black/5 dark:border-white/5"
                      />
                    ...
                    <div className="flex items-center gap-2">
                      <Checkbox
                        checked={stagedConvertedFilter === "false"}
                        onCheckedChange={() =>
                          setStagedConvertedFilter("false")
                        }
                        className="rounded-lg border-black/5 dark:border-white/5"
                      />
                      <span className="text-xs text-primary">
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent alignOffset={-5}>
                    <DateRangeFilter
                      ...
                      maxDate={new Date()}
                      valueStart={startedAtStart}
                      valueEnd={startedAtEnd}
                      onChange={(start, end) => onStartedAtChange(start, end)}
                    />
                  </DropdownMenuSubContent>
                </DropdownMenuSub>
              )}
              ...
              <div className="pt-1 px-2 flex gap-2">
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    if (
                      onToggleDeviceType &&
                      JSON.stringify(stagedDeviceTypes) !==
                        JSON.stringify(selectedDeviceTypes)
                    ) {
                      stagedDeviceTypes.forEach(onToggleDeviceType);
                    }
                    if (
                      onConvertedFilterChange &&
                      stagedConvertedFilter !== convertedFilter
                    ) {
                      onConvertedFilterChange(stagedConvertedFilter);
                    }
                    setFiltersOpen(false);
                  }}
                >
