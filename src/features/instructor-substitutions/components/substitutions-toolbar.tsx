...
const sortOptions = [
  { value: "requestedAt.desc", label: "Newest first" },
  { value: "requestedAt.asc", label: "Oldest first" },
  { value: "classTime.asc", label: "Class time ↑" },
  { value: "classTime.desc", label: "Class time ↓" },
];

const PRIMARY_COLUMN_ID = "class";

export interface SubstitutionsToolbarProps {
  search: string;
...
export function SubstitutionsToolbar({
  search,
  onSearchChange,
  selectedStatuses,
  sortValue,
            ...
            >
                ...
                >
                  <div className="space-y-1">
                    {statusOptions.map((opt) => (
                      ...
                      >
                        <Checkbox
                          checked={stagedStatuses.includes(opt.value)}
                          onCheckedChange={() =>
                            setStagedStatuses((prev) =>
                              prev.includes(opt.value)
                                ? prev.filter((v) => v !== opt.value)
              ...
              <div className="pt-1">
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onApplyFilters({ statuses: stagedStatuses });
