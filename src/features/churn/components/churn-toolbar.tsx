...
const sortOptions = [
  { value: "score.desc", label: "Highest risk first" },
  { value: "score.asc", label: "Lowest risk first" },
  { value: "name.asc", label: "Name A → Z" },
  { value: "name.desc", label: "Name Z → A" },
  { value: "calculatedAt.desc", label: "Most recently calculated" },
];

const PRIMARY_COLUMN_ID = "name";

export interface ChurnToolbarProps {
  search: string;
  ...
  onApplyFilters: (filters: { riskLevels: string[] }) => void;
}

export function ChurnToolbar({
  search,
  onSearchChange,
  selectedRiskLevels,
  sortValue,
  ...
  const [stagedRisk, setStagedRisk] =
    React.useState<string[]>(selectedRiskLevels);
  ...
  React.useEffect(() => {
    setStagedRisk(selectedRiskLevels);
  }, [selectedRiskLevels]);
  ...
  const hasFilters = selectedRiskLevels.length > 0;

  return (
    <div className="flex justify-between w-full items-center py-4">
      <div className="flex items-center gap-2 w-full">
        <div className="flex w-80 items-center bg-background transition duration-250 relative hover:bg-primary-foreground/50 hover:text-black rounded-lg h-8.5">
            ...
            >
                ...
                >
                  <div className="space-y-1">
                    {riskOptions.map((opt) => (
                      ...
                      >
                        <Checkbox
                          checked={stagedRisk.includes(opt.value)}
                          onCheckedChange={() =>
                            setStagedRisk((prev) =>
                              prev.includes(opt.value)
                                ? prev.filter((v) => v !== opt.value)
              ...
              <div className="pt-1">
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onApplyFilters({ riskLevels: stagedRisk });
