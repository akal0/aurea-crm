...
export interface IntroOffersToolbarProps {
  search: string;
  ...
  onApplyFilters: (filters: { types: string[]; active: string[] }) => void;
}

export function IntroOffersToolbar({
  search,
  onSearchChange,
  selectedTypes,
  selectedActive,
  ...
  initialColumnOrder,
  onApplyFilters,
}: IntroOffersToolbarProps) {
  const [searchInput, setSearchInput] = React.useState(search);
  const debouncedSearch = useDebouncedCallback(onSearchChange, 500);
  const [stagedTypes, setStagedTypes] = React.useState<string[]>(selectedTypes);
  const [stagedActive, setStagedActive] =
    React.useState<string[]>(selectedActive);
  ...
  React.useEffect(() => {
    setStagedActive(selectedActive);
  }, [selectedActive]);

  const hasFilters = selectedTypes.length > 0 || selectedActive.length > 0;
        ...
        <div className="flex w-80 items-center bg-background transition duration-250 relative hover:bg-primary-foreground/50 rounded-lg h-8.5">
          ...
          <DropdownMenu open={filtersOpen} onOpenChange={setFiltersOpen}>
            ...
            >
              ...
              <DropdownMenuSub>
                <DropdownMenuSubTrigger>Offer type</DropdownMenuSubTrigger>
                <DropdownMenuSubContent
                  className="rounded-lg bg-background border border-black/10 dark:border-white/5 p-3 pt-2 w-[240px] ml-2.5"
                  alignOffset={-5}
                >
                  <div className="space-y-1">
                    {offerTypeOptions.map((opt) => (
                      ...
                      >
                        <Checkbox
                          checked={stagedTypes.includes(opt.value)}
                          onCheckedChange={() =>
                            setStagedTypes((prev) =>
                              prev.includes(opt.value)
                                ? prev.filter((v) => v !== opt.value)
              ...
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent
                  className="rounded-lg bg-background border border-black/10 dark:border-white/5 p-3 pt-2 w-[180px] ml-2.5"
                  alignOffset={-5}
                >
                  <div className="space-y-1">
                    {activeOptions.map((opt) => (
                      ...
                      >
                        <Checkbox
                          checked={stagedActive.includes(opt.value)}
                          onCheckedChange={() =>
                            setStagedActive((prev) =>
                              prev.includes(opt.value)
                                ? prev.filter((v) => v !== opt.value)
